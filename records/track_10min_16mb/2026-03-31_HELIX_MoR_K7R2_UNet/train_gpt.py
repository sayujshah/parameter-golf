...
class Hyperparameters:
    ...
    num_unique_blocks = int(os.environ.get("NUM_UNIQUE_BLOCKS", 7))
    num_iterations = int(os.environ.get("NUM_ITERATIONS", 2))
    num_layers = num_unique_blocks * num_iterations  # virtual layer count (derived)
    ...
    ve_layers = os.environ.get("VE_LAYERS", "12,13")  # last 2 virtual layers of 14 (decoder blocks 5,6)
...
class Block(nn.Module):
    def __init__(
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None, v_attn_scale: Tensor | None = None, v_mlp_scale: Tensor | None = None, v_resid_mix: Tensor | None = None, v_ln_scale_factor: float | None = None) -> tuple[Tensor, Tensor | None]:
        # MoR: use per-virtual-layer scalars when provided, else fall back to block's own
        mix = (v_resid_mix if v_resid_mix is not None else self.resid_mix).to(dtype=x.dtype)
        lsf = v_ln_scale_factor if v_ln_scale_factor is not None else self.ln_scale_factor
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out, raw_v = self.attn(self.attn_norm(x_in) * lsf, q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0)
        attn_s = (v_attn_scale if v_attn_scale is not None else self.attn_scale).to(dtype=x_in.dtype)
        x_out = x_in + attn_s[None, None, :] * attn_out
        mlp_s = (v_mlp_scale if v_mlp_scale is not None else self.mlp_scale).to(dtype=x_out.dtype)
        x_out = x_out + mlp_s[None, None, :] * self.mlp(self.mlp_norm(x_out) * lsf, up_w, down_w)
...
class GPT(nn.Module):
    def __init__(
        self,
        vocab_size: int,
        num_layers: int,            # = num_unique_blocks * num_iterations (virtual total)
        num_unique_blocks: int,     # K: unique weight sets
        num_iterations: int,        # R: times to iterate over unique blocks
        model_dim: int,
        num_heads: int,
        num_kv_heads: int,
        mlp_mult: int,
        ...
        # MoR parameters
        self.num_unique_blocks = num_unique_blocks   # K
        self.num_iterations = num_iterations          # R
        self.num_layers = num_layers                  # K * R (virtual total, kept for compat)
        # Embeddings
        self.tok_emb = nn.Embedding(vocab_size, model_dim)
        self.bigram = BigramHashEmbedding(bigram_vocab_size, bigram_dim, model_dim) if bigram_vocab_size > 0 else None
        self.smear = SmearGate(model_dim)
        # U-Net skip connections: encoder = first iteration (K blocks), decoder = remaining iterations
        # With R=2: encoder has K blocks, decoder has K blocks → K skip weights
        self.num_skip_weights = num_unique_blocks
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))
        # Parameter banks: K unique weight sets (batched Muon)
        K = num_unique_blocks
        ...
        # K unique blocks (small params only: norms, q_gain, rotary)
        self.blocks = nn.ModuleList(
            [
                Block(
                    model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                    layer_idx=bi, ln_scale=ln_scale, dtg=dtg,
                    gated_attention=gated_attention, value_residual=value_residual,
                )
                for bi in range(K)
            ]
        ...
        # XSA on last xsa_last_n UNIQUE blocks (applied every iteration)
        if xsa_last_n > 0:
            for bi in range(max(0, K - xsa_last_n), K):
                self.blocks[bi].attn.use_xsa = True
        # Per-virtual-layer scalars for MoR iterations r > 0 (K*(R-1) extra sets)
        # Encoder iteration (r=0) uses each block's own attn_scale/mlp_scale/resid_mix
        # Decoder iterations (r=1..R-1) use these per-virtual-layer scalars
        extra_virt = K * (num_iterations - 1)
        self.v_attn_scale = nn.ParameterList([
            nn.Parameter(torch.ones(model_dim, dtype=torch.float32)) for _ in range(extra_virt)
        ])
        self.v_mlp_scale = nn.ParameterList([
            nn.Parameter(torch.ones(model_dim, dtype=torch.float32)) for _ in range(extra_virt)
        ])
        self.v_resid_mix = nn.ParameterList([
            nn.Parameter(torch.stack([torch.ones(model_dim), torch.zeros(model_dim)]).float())
            for _ in range(extra_virt)
        ])
        # LN scale factors for all virtual layers (Python floats, not Parameters)
        self.virtual_ln_scale_factors = [
            1.0 / math.sqrt(vi + 1) if ln_scale else 1.0
    ...
    def _mor_forward(self, input_ids: Tensor) -> Tensor:
        """MoR forward pass: K unique blocks × R iterations with U-Net skip connections.
        Encoder = first iteration (r=0): saves K skip tensors.
        Decoder = iterations r=1..R-1: injects skips in reverse order (symmetric U-Net).
        Returns final hidden state x [B, T, D]."""
        K = self.num_unique_blocks
        R = self.num_iterations
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        """Return logits [B, T, V] for sliding window eval / TTT."""
        x = self._mor_forward(input_ids)
        x = self.final_norm(x)
        if self.tie_embeddings:
...
def eval_val_sliding(
    ...
    base_model.eval()
    compiled_logits = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=_COMPILE_FULLGRAPH) if _COMPILE_FULLGRAPH else base_model.forward_logits
    with torch.inference_mode():
...
def _unbank_state_dict(sd: dict[str, Tensor], num_unique_blocks: int) -> dict[str, Tensor]:
    """Convert 3D bank tensors (K unique blocks) into individual 2D tensors."""
    out: dict[str, Tensor] = {}
    K = num_unique_blocks
    for name, tensor in sd.items():
        if name == "qo_bank":
            for i in range(K):
                ...
                out[f"blocks.{i}.attn.proj.weight"] = tensor[K + i]
        elif name == "kv_bank":
            for i in range(K):
                out[f"blocks.{i}.attn.c_k.weight"] = tensor[i]
                out[f"blocks.{i}.attn.c_v.weight"] = tensor[K + i]
...
def _rebank_state_dict(sd: dict[str, Tensor], num_unique_blocks: int, template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
...
def main() -> None:
    ...
    compiled_model = torch.compile(base_model, dynamic=False, fullgraph=_COMPILE_FULLGRAPH) if _COMPILE_FULLGRAPH else base_model
    ...
    # MoR per-virtual-layer scalars for decoder iterations
    scalar_params.extend(list(base_model.v_attn_scale.parameters()))
    scalar_params.extend(list(base_model.v_mlp_scale.parameters()))
    scalar_params.extend(list(base_model.v_resid_mix.parameters()))
    ...
    unbanked_sd = _unbank_state_dict(sd_cpu, args.num_unique_blocks)
    ...
    deq_state = _rebank_state_dict(deq_unbanked, args.num_unique_blocks, sd_cpu)
    eval_model = GPT(
        vocab_size=args.vocab_size, num_layers=args.num_layers,
        num_unique_blocks=args.num_unique_blocks, num_iterations=args.num_iterations,
        model_dim=args.model_dim,
    ...
    compiled_eval = torch.compile(eval_model, dynamic=False, fullgraph=_COMPILE_FULLGRAPH) if _COMPILE_FULLGRAPH else eval_model
    torch.cuda.synchronize()
